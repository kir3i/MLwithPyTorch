...
class CNN(nn.Module):
    ...
    def forward(self, x):
        out = self.layer3(self.layer2(self.layer1(x)))  # go through convolution layer
...
with torch.no_grad():
    ...
    print(f'Accuracy: {accuracy*100:.3f}')  # CUDA로 학슴했을 때 99.020%의 정확도를 보인다!!! (11번에 비해 layer 추가)